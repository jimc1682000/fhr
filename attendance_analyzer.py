...
class AttendanceAnalyzer:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        self.forget_punch_usage: Dict[str, int] = defaultdict(
            int
        )  # 追蹤每月忘刷卡使用次數 {年月: 次數}
    ...
    def set_progress_callback(
        self, cb: Optional[Callable[[str, int, Optional[int]], None]] = None
    ) -> None:
    ...
    def _try_load_from_gov_api(self, year: int) -> bool:
        ...
        url = (
            'https://data.gov.tw/api/v1/rest/datastore_search?resource_id=W2&filters={"date":"%s"}'
            % year
        )
        parsed = urlparse(url)
    ...
    def _update_processing_state(self) -> None:
        ...
        range_info = {
            ...
            "end_date": end_date,
            "source_file": (
                os.path.basename(sys.argv[1]) if len(sys.argv) > 1 else "unknown"
            ),
    ...
    def generate_report(self) -> str:
        ...
        if weekday_leave_issues:
            ...
            for i, issue in enumerate(weekday_leave_issues, 1):
                ...
                report.append(
                    f"{i}. **{issue.date.strftime('%Y/%m/%d')} ({weekday_name})** - 📝 {issue.description}"
                )
            report.append("")
    ...
    def export_csv(self, filepath: str) -> None:
        ...
        csv_exporter.save_csv(
            filepath, self.issues, self.incremental_mode, status_tuple
        )
