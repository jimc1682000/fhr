...
依賴:
  ...
  - state cache 用於跳過已申請的條目 (auto-syncs if older than --sync-max-age)
    ...
    parser.add_argument("--no-sync", action="store_true",
                        help="跳過 portal-sync (信任現有 state cache)")
    parser.add_argument("--sync-max-age-hours", type=int, default=4,
...
def _is_early_arrival(date: str, schedule_start: str,
                      latest_checkin: str, attendance: dict) -> tuple[bool, int]:
...
def _format_entry(entry: dict, attendance: dict,
                  *, schedule_start: str, latest_checkin: str,
                  expected_checkout: str | None = None) -> str:
    st = entry["start_time"]
    et = entry["end_time"]
    base = (
        f"{entry['date']} {_fmt_time(st)}-{_fmt_time(et)} "
        f"({entry['hours']}h)"
    )
    rec = attendance.get(entry["date"])
    if rec:
        base += (
            f"  | 實際 上班 {rec.get('上班', '—')}  下班 {rec.get('下班', '—')}"
        )
...
def _maybe_sync(args: argparse.Namespace, base_url: str, logger) -> None:
    ...
    if age_hours is None or age_hours > args.sync_max_age_hours:
        logger.info("ℹ️ state cache 已老 (%.1fh) — 先跑 portal-sync",
        ...
        sm.replace_applied_forms(
            args.user, entries, datetime.now().isoformat(timespec="seconds"),
        )
        sm.save_state()


def _interactive_overtime(
    *, overtime: list[dict], attendance: dict, completed: set[str],
    schedule_start: str, latest_checkin: str,
    saved_plan: dict, plan: dict, plan_path: Path,
    logger,
) -> None:
    if not overtime:
        return
    ...
    for i, entry in enumerate(overtime, 1):
        ...
        if key in completed:
            logger.info("[%d/%d] %s ✅ 已申請,略過", i, len(overtime),
                        _format_entry(entry, attendance,
        ...
        early, delta = _is_early_arrival(entry["date"], schedule_start,
                                         latest_checkin, attendance)
        early_hint = None
        if early:
            sh, sm = (int(x) for x in schedule_start.split(":"))
            shift_min = delta
        ...
        while True:
            ...
            if action == "y":
                ...
                reason = (
                    input(f"  加班原因 [{default_reason}]: ").strip()
                    or default_reason
                )
                plan["overtime"].append({"entry": entry, "action": "submit",
                                         "key": key, "reason": reason})
                _save_plan(plan_path, plan)
                break
            if action == "s":
                plan["overtime"].append({"entry": entry, "action": "skip",
                                         "key": key})
                _save_plan(plan_path, plan)
                break
...
def _interactive_leave(
    *, leave: list[dict], attendance: dict, completed: set[str],
    schedule_start: str, latest_checkin: str,
    saved_plan: dict, plan: dict, plan_path: Path,
    proxy: str | None, allocations: dict[str, str],
    leave_type_map: dict[str, str], logger,
) -> None:
    if not leave:
        return
    common_keys = ["27", "30", "1", "2", "5", "18"]
    common_display = "  ".join(
        f"{k}={leave_type_map.get(k, '?')}" for k in common_keys
    )
    ...
    for i, entry in enumerate(leave, 1):
        ...
        if key in completed:
            logger.info("[%d/%d] %s ✅ 已申請,略過", i, len(leave),
                        _format_entry(entry, attendance,
                                      schedule_start=schedule_start,
                                      latest_checkin=latest_checkin))
            plan["leave"].append({"entry": entry, "action": "skip",
                                  "already_done": True})
            continue
        if key in saved_plan:
            d = saved_plan[key]
            plan["leave"].append({**d, "entry": entry})
            logger.info("[%d/%d] %s ↩️  沿用 (%s / %s)", i, len(leave),
                        _format_entry(entry, attendance,
                                      schedule_start=schedule_start,
                                      latest_checkin=latest_checkin),
                        d.get("action"), d.get("leave_type", ""))
            continue
        ...
        while True:
            ...
            choice = (
                input(f"  假別 [{common_display}  ?=展開] ({default_code}): ")
                .strip() or default_code
            )
            if choice == "?":
            ...
            if action == "y":
                ...
                reason = (
                    input(f"  請假原因 [{default_reason}]: ").strip()
                    or default_reason
                )
                entry_proxy = proxy
                if "異地辦公" not in leave_type and proxy:
                    new_proxy = (
                        input(f"  代理人 [{proxy},Enter 保留]: ").strip()
                    )
                    if new_proxy:
                        entry_proxy = new_proxy
                plan["leave"].append({
                    "entry": entry, "action": "submit", "key": key,
...
LEAVE_TYPE_MAP: dict[str, str] = {
    "1": "特休假", "2": "事假", "3": "半薪病假", "4": "住院病假",
    "5": "有薪病假", "6": "安胎假", "7": "婚假", "8": "訂婚假",
    "9": "陪產假", "10": "陪產假-o", "11": "八日喪假", "12": "六日喪假",
    "13": "三日喪假", "14": "公假-教召", "15": "公假-訓練", "16": "公假-公出",
    "17": "公傷病假", "18": "忘刷忘帶卡", "19": "國內出差", "20": "國外出差",
    "21": "行政假", "22": "榮譽假", "23": "產檢假-新", "24": "防疫照顧假",
    "25": "防疫假", "26": "疫苗接種假", "27": "異地辦公", "28": "謀職假",
    "29": "社團參與假", "30": "補休假", "31": "生日假", "32": "遲到早退",
    "33": "出勤異常扣薪",
}
...
def run(args: argparse.Namespace) -> None:
    ...
    for kind, entries in [("overtime", sm.get_applied_forms(args.user, "overtime")),
    ...
    if args.auto:
        for e in overtime:
            ...
            if key in completed:
                plan["overtime"].append({"entry": e, "action": "skip",
                                         "already_done": True})
                continue
            plan["overtime"].append({"entry": e, "action": "submit",
                                     "key": key,
                                     "reason": e.get("reason", "工作需要")})
        for e in leave:
            key = _entry_key(e, "leave")
            if key in completed:
                plan["leave"].append({"entry": e, "action": "skip",
                                      "already_done": True})
                continue
        ...
        _interactive_overtime(
            overtime=overtime, attendance=attendance, completed=completed,
            schedule_start=config.schedule_start,
            latest_checkin=config.latest_checkin,
            saved_plan=saved_plan, plan=plan, plan_path=plan_path,
            logger=logger,
        )
        _interactive_leave(
            leave=leave, attendance=attendance, completed=completed,
            schedule_start=config.schedule_start,
            latest_checkin=config.latest_checkin,
            saved_plan=saved_plan, plan=plan, plan_path=plan_path,
            proxy=args.proxy, allocations=allocations,
            leave_type_map=LEAVE_TYPE_MAP, logger=logger,
        )
    ...
    logger.info("\n%s\n📋 送出確認\n%s", "=" * 50, "=" * 50)
    for p in submit_ot:
        logger.info("  加班 %s  reason: %s",
                    _format_entry(p["entry"], attendance,
                                  schedule_start=config.schedule_start,
                                  latest_checkin=config.latest_checkin),
                    p["reason"])
    for p in submit_lv:
        logger.info("  請假 %s [%s] reason: %s",
                    _format_entry(p["entry"], attendance,
                                  schedule_start=config.schedule_start,
                                  latest_checkin=config.latest_checkin),
                    p["leave_type"], p["reason"])
        ...
        if ok and not args.dry_run:
            sm.replace_applied_forms(
                ...
                {"overtime": sm.get_applied_forms(args.user, "overtime") + [{
                    **plan_entry["entry"],
                    "leave_type": None, "status": "已送出 (本機新增)",
                }], "leave": sm.get_applied_forms(args.user, "leave")},
